// Copyright Epic Games, Inc. All Rights Reserved.
/*===========================================================================
	Generated code exported from UnrealHeaderTool.
	DO NOT modify this manually! Edit the corresponding .h files instead!
===========================================================================*/

#include "UObject/GeneratedCppIncludes.h"
#include "Farm/Public/Bush.h"
PRAGMA_DISABLE_DEPRECATION_WARNINGS
void EmptyLinkFunctionForGeneratedCodeBush() {}
// Cross Module References
	ENGINE_API UClass* Z_Construct_UClass_AActor();
	FARM_API UClass* Z_Construct_UClass_ABush();
	FARM_API UClass* Z_Construct_UClass_ABush_NoRegister();
	UPackage* Z_Construct_UPackage__Script_Farm();
// End Cross Module References
	void ABush::StaticRegisterNativesABush()
	{
	}
	IMPLEMENT_CLASS_NO_AUTO_REGISTRATION(ABush);
	UClass* Z_Construct_UClass_ABush_NoRegister()
	{
		return ABush::StaticClass();
	}
	struct Z_Construct_UClass_ABush_Statics
	{
		static UObject* (*const DependentSingletons[])();
#if WITH_METADATA
		static const UECodeGen_Private::FMetaDataPairParam Class_MetaDataParams[];
#endif
		static const FCppClassTypeInfoStatic StaticCppClassTypeInfo;
		static const UECodeGen_Private::FClassParams ClassParams;
	};
	UObject* (*const Z_Construct_UClass_ABush_Statics::DependentSingletons[])() = {
		(UObject* (*)())Z_Construct_UClass_AActor,
		(UObject* (*)())Z_Construct_UPackage__Script_Farm,
	};
	static_assert(UE_ARRAY_COUNT(Z_Construct_UClass_ABush_Statics::DependentSingletons) < 16);
#if WITH_METADATA
	const UECodeGen_Private::FMetaDataPairParam Z_Construct_UClass_ABush_Statics::Class_MetaDataParams[] = {
		{ "IncludePath", "Bush.h" },
		{ "ModuleRelativePath", "Public/Bush.h" },
	};
#endif
	const FCppClassTypeInfoStatic Z_Construct_UClass_ABush_Statics::StaticCppClassTypeInfo = {
		TCppClassTypeTraits<ABush>::IsAbstract,
	};
	const UECodeGen_Private::FClassParams Z_Construct_UClass_ABush_Statics::ClassParams = {
		&ABush::StaticClass,
		"Engine",
		&StaticCppClassTypeInfo,
		DependentSingletons,
		nullptr,
		nullptr,
		nullptr,
		UE_ARRAY_COUNT(DependentSingletons),
		0,
		0,
		0,
		0x009000A4u,
		METADATA_PARAMS(UE_ARRAY_COUNT(Z_Construct_UClass_ABush_Statics::Class_MetaDataParams), Z_Construct_UClass_ABush_Statics::Class_MetaDataParams)
	};
	UClass* Z_Construct_UClass_ABush()
	{
		if (!Z_Registration_Info_UClass_ABush.OuterSingleton)
		{
			UECodeGen_Private::ConstructUClass(Z_Registration_Info_UClass_ABush.OuterSingleton, Z_Construct_UClass_ABush_Statics::ClassParams);
		}
		return Z_Registration_Info_UClass_ABush.OuterSingleton;
	}
	template<> FARM_API UClass* StaticClass<ABush>()
	{
		return ABush::StaticClass();
	}
	DEFINE_VTABLE_PTR_HELPER_CTOR(ABush);
	ABush::~ABush() {}
	struct Z_CompiledInDeferFile_FID_Farm_Source_Farm_Public_Bush_h_Statics
	{
		static const FClassRegisterCompiledInInfo ClassInfo[];
	};
	const FClassRegisterCompiledInInfo Z_CompiledInDeferFile_FID_Farm_Source_Farm_Public_Bush_h_Statics::ClassInfo[] = {
		{ Z_Construct_UClass_ABush, ABush::StaticClass, TEXT("ABush"), &Z_Registration_Info_UClass_ABush, CONSTRUCT_RELOAD_VERSION_INFO(FClassReloadVersionInfo, sizeof(ABush), 1542764668U) },
	};
	static FRegisterCompiledInInfo Z_CompiledInDeferFile_FID_Farm_Source_Farm_Public_Bush_h_2619210320(TEXT("/Script/Farm"),
		Z_CompiledInDeferFile_FID_Farm_Source_Farm_Public_Bush_h_Statics::ClassInfo, UE_ARRAY_COUNT(Z_CompiledInDeferFile_FID_Farm_Source_Farm_Public_Bush_h_Statics::ClassInfo),
		nullptr, 0,
		nullptr, 0);
PRAGMA_ENABLE_DEPRECATION_WARNINGS
